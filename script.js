...
const initUIEffects = () => {
  ...
  const type = () => {
    ...
    typingEl.textContent = current.substring(
      0,
      isDeleting ? charIdx - 1 : charIdx + 1,
    );
...
const initLightbox = () => {
  ...
  document.querySelectorAll(".view-project-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      currentImages = Array.from(
        p.querySelectorAll(".carousel-slides img"),
      ).map((i) => i.src);
      currentIdx = 0;

      lbTitle.textContent = p.getAttribute("data-title") || "Project";
      lbDesc.innerHTML = parseDescription(
        p.getAttribute("data-description") || "",
      );
      lbGithub.href = p.getAttribute("data-github") || "#";
      lbGithub.style.display = p.getAttribute("data-github")
        ? "inline-flex"
        : "none";

      updateTechStack(p.getAttribute("data-tech"), lbTech);

      lb.querySelector(".lightbox-gallery").className =
        `lightbox-gallery ${p.getAttribute("data-type") || ""}-frame`;
      open();
    });
  });
  ...
  const parseDescription = (raw) => {
    ...
    lines.forEach((l, i) => {
      if (l.startsWith("Key Features:")) {
        ...
        html +=
          i === 0 ? `<p class="lightbox-main-desc">${l}</p>` : `<p>${l}</p>`;
      }
    });
  ...
  const updateTechStack = (json, container) => {
    try {
      ...
      container.innerHTML = data
        .map(
          (t) =>
            `<div class="tech-tag"><i class="${t.icon}"></i><span>${t.name}</span></div>`,
        )
        .join("");
    } catch (e) {
      container.innerHTML = "";
  ...
  const show = (idx) => {
    ...
    lbDots.innerHTML = currentImages
      .map(
        (_, i) => `<span class="${i === currentIdx ? "active" : ""}"></span>`,
      )
      .join("");
    lbDots
      .querySelectorAll("span")
      .forEach((s, i) => (s.onclick = () => show(i)));
  };
...
const initContactForm = () => {
  ...
  try {
    ...
    const key =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZ6cnJ3Ynp0aHBiYmZrbWJpZm5lIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM0ODgyNzgsImV4cCI6MjA4OTA2NDI3OH0.rHBYIw8KrUXPAcBA2zYFI3RCcBO-apA5Jms8hmbFFL8";
